    ...
    <div class="scheduled-wrapper">
        ...
        <div class="schedule-card" v-if="add">
            <el-form ref="weekScheduleForm" :model="weekScheduleForm" :rules="rules" label-position="right" label-width="100px">
                ...
                <el-form-item label="例外日期：" prop="trainDate" style="margin-bottom:22px;">
                    <el-date-picker v-model="weekScheduleForm.trainDate" type="date" placeholder="选择日期" :picker-options="pickerOptions" :editable="false" @change="handleDataChange" :clearable="false">
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="时段管理：" v-if="!isCancel && weekScheduleForm.trainDate">
                    <v-timeBlock :periods="weekScheduleForm.trainTimes" ref="exceptTimes"></v-timeBlock>
                </el-form-item>
        ...
        <div class="schedule-card" v-for="(except, index) in exceptTimes" :key="except.trainDate">
            <template v-if="edit && editIndex === index">
                <el-form :ref="'weekScheduleForm_'+index" :model="weekScheduleForm" :rules="rules" label-position="right" label-width="100px">
                    ...
                    <el-form-item label="时段管理：" v-if="!isCancel && weekScheduleForm.trainDate">
                        <v-timeBlock :periods="weekScheduleForm.trainTimes" :ref="'exceptTimes'+index"></v-timeBlock>
                    </el-form-item>
                    <div class="dialog-footer">
            ...
            <template v-else>
                ...
                <div class="day-item">
                    <h4 class="week">{{convertDate(except.trainDate,'yyyy-MM-dd（EE）')}}</h4>
                    <span v-if="except.type==='cancel'">取消当天</span>
                    <template v-if="except.type!=='cancel'">
                        <div v-for="(time, i) in except.trainTimes" :key="i" class="time-itms">
...
export default {
    ...
    methods: {
        ...
        handleDataChange(val) {
            if (this.exceptTimes!==null&&this.exceptTimes.findIndex(x => x.trainDate === val) !== -1) {
                this.showCenterTip('该日期已设置，请重新选择日期！', 'error');
                this.weekScheduleForm.trainDate = null;
                return;
            }
            if (this.isCancel) return;
            if (val && !this.weekScheduleForm.trainTimes.length) {
                this.weekScheduleForm.trainTimes = INIT_PERIODS.slice();
            }
        },
        // 是否取消当天
        cancelChange() {
            if (this.isCancel) {
                this.weekScheduleForm.trainTimes = [];
            } else if (this.weekScheduleForm.trainDate && !this.weekScheduleForm.trainTimes.length) {
                this.weekScheduleForm.trainTimes = INIT_PERIODS.slice();
            }
        },
        resetForm(formName) {
        ...
        submitItem(formName) {
            ...
            $submitForm.validate((valid) => {
                ...
                if (!this.isCancel) {
                    ...
                    this.weekScheduleForm.trainTimes = timeBlock.periods;
                }
                let item = {
                    trainDate: this.formatDate(this.weekScheduleForm.trainDate, 'yyyy-MM-dd'),
                    type: this.weekScheduleForm.type,
                    trainTimes: this.weekScheduleForm.trainTimes
                }
        ...
        HandleEdit(index, form) {
            ...
            this.weekScheduleForm.trainDate = this.convertToDate(this.weekScheduleForm.trainDate);
            this.edit = true;
        },
        HandleDel(index) {
            var self = this;
