<template>
    <div class="edit-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'index',name: '会员作品管理' },{name:title}]"></v-pageheader>
            ...
            <el-form ref="cultureteamForm" :model="cultureteamForm" :rules="rules" label-position="right" label-width="100px" class="m-form">
                <el-row :gutter="20">
                    <el-col :span="14">
                        ...
                        <!-- <el-form-item label="会员负责人" prop="contactName">
                ...
                <el-form-item label="会员介绍" prop="desc">
...
const DIALOG = {
    add: { title: '新增会员', flag: 'Add' },
    edit: { title: '编辑会员', flag: 'Edit' }
};
export default {
    data() {
        return {
    ...
    methods: {
            ...
            this.$refs['cultureteamForm'].validate((valid) => {
                if (valid) {
                    ...
                    if (this.flag === DIALOG.add.flag) {
                        ...
                        Api.memberWorks.addmemberWorks(newForm).then(this.callback);
                    } else if (this.flag === DIALOG.edit.flag) {
                        newForm.id = this.cultureteamForm.id;
                        Api.memberWorks.modifymemberWorks(newForm.id, newForm).then(this.callback);
                    }
                }
            })
        },
        ...
        handleUpload(formData) {
            this.cultureteamForm.coverPic = 'e/3/5/c/e35ca5d6-12c0-4c48-ad0d-95581426be3c.png';
        ...
        getDetail() {
            Api.memberWorks.getmemberWorksDetail(this.culid).then((res) => {
