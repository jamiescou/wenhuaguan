<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ name: '会员作品' }]"></v-pageheader>
        ...
        <div class="table-container">
            <el-table :data="dataList" border stripe v-loading.body="loading">
                ...
                <!-- <el-table-column prop="contactName" label="会员负责人"></el-table-column> -->
                <el-table-column prop="createTime" label="会员创建时间"></el-table-column>
                ...
                <el-table-column label="操作" width="400px" align="center">
                    <template scope="scope">
                        ...
                        <a class="btn-act" @click="mien(scope.$index, scope.row)">会员作品</a>
                        <!-- <a class="btn-act" @click="person(scope.$index, scope.row)">会员成员</a> -->
...
export default {
    ...
    methods: {
        ...
        loadData() {
            ...
            Api.memberWorks.getmemberWorksList(str, this.page, this.size).then((res) => {
                console.log('res=============>>>>>>>', res)
        ...
        del(index, row) {
            ...
            self.delConfirm('会员作品', () => {
                Api.memberWorks.delmemberWorks(row.id).then(self.callback);
            });
        },
        publish(index, row) {
            let msg = row.isPublish ? '是否确认取消上架？' : '确认上架该会员作品？';
            this.$confirm(msg, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then((res) => {
                Api.memberWorks.publishmemberWorks(row.id, !row.isPublish).then(this.callback);
                this.loadData();
            });
        },
        callback() {
        ...
        // 置顶
        handleRecomd(row) {
            let msg = row.isTop ? '确认取消置顶？' : '确认置顶该会员作品？';
            this.$confirm(msg, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then((res) => {
                Api.memberWorks.topmemberWorks(row.id, !row.isTop).then((res) => {
                    this.loadData();
                });
            });
        },
