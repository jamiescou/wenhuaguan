<template>
    <div class="mien-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'index',name:'会员作品'},{name:'会员作品管理'},{name:title}]"></v-pageheader>
        <div class="form-wrapper">
            <el-form ref="digitInfoForm" :model="digitInfoForm" :rules="rules" label-position="right" label-width="120px" class="m-form">
                <el-form-item label="作品标题：" prop="title">
                    <el-input v-model="digitInfoForm.title" placeholder="作品标题"></el-input>
                </el-form-item>
                <el-col :span="24">
                ...
                <el-col :span="24">
                    <el-form-item label="作品图片：" v-show="repic">
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            this.$refs['digitInfoForm'].validate((valid) => {
                if (valid) {
                    ...
                    if (this.flag === 'add') {
                        Api.memberWorks.addMien(this.id, newForm).then(this.callback);
                    } else {
                        Api.memberWorks.editMien(this.id, this.$route.query.mid, newForm).then(this.callback);
                    }
                }
            });
        },
        ...
        getDetail() {
            Api.memberWorks.detailMien(this.id, this.$route.query.mid).then((res) => {
