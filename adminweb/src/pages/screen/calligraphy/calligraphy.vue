<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ name: '书法资源管理' }]"></v-pageheader>
            ...
            <el-table :data="dataList" border stripe v-loading.body="loading">
                <el-table-column prop="name" label="资源名称">
                    <template scope="scope">
                        <router-link :to="{path:'calligraphyview', query: {id: scope.row.id}}" class="u-link">
                            {{scope.row.name}}
                        </router-link>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        // 添加
        handleAdd() {
            this.$router.push({ path: 'calligraphyadd', query: { flag: 'add' } });
        },
        // 编辑
        edit(index, row) {
            this.$router.push({ path: 'calligraphyadd', query: { id: row.id, flag: 'edit' } })
        },
        // 删除
        del(index, row) {
            let self = this;
            self.delConfirm('书法资源', () => {
                Api.screen.delScreen(row.id).then(self.callback);
            });
        },
        // 上下架
        publish(index, row) {
            let msg = row.isPublish ? '是否确认取消发布？' : '确认发布书法资源？';
            this.$confirm(msg, '提示', {
