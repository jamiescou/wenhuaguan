<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ name: '文化名人管理' }]"></v-pageheader>
            ...
            <el-table :data="dataList" border stripe v-loading.body="loading">
                <el-table-column prop="name" label="名称">
                    <template scope="scope">
                        <router-link :to="{path:'culturalManview', query: {id: scope.row.id}}" class="u-link">
                            {{scope.row.name}}
                        </router-link>
                    </template>
                </el-table-column>
                <!-- <el-table-column prop="author" label="作者"></el-table-column> -->
                <el-table-column prop="brief" label="名人简介"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        // 添加
        handleAdd() {
            this.$router.push({ path: 'culturalManadd', query: { flag: 'add' } });
        },
        // 编辑
        edit(index, row) {
            this.$router.push({ path: 'culturalManadd', query: { id: row.id, flag: 'edit' } })
        },
        // 删除
        del(index, row) {
            let self = this;
            self.delConfirm('文化名人', () => {
                Api.screen.delScreen(row.id).then(self.callback);
            });
        },
        // 上下架
        publish(index, row) {
            let msg = row.isPublish ? '是否确认取消发布？' : '确认发布文化名人？';
            this.$confirm(msg, '提示', {
