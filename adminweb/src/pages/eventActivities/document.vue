...
const DIALOG = {
    add: { title: '添加赛事作品活动', flag: 'Add' },
    edit: { title: '编辑赛事作品活动', flag: 'Edit' }
};
export default {
    data() {
          /**
        ...
        return {
            ...
            rules: {
                'name': [vRules.required, vRules.titlePattern()],
    ...
    methods: {
        ...
        handleUpload(formData) {
            // this.activityForm.coverPic = 'e/3/5/c/e35ca5d6-12c0-4c48-ad0d-95581426be3c.png'
                  ...
                  // 日期时间格式化
                    ...
                    if (this.flag === DIALOG.add.flag) {
                       ...
                       newForm.unitId = this.$store.getters.user.orgUnit.id;
                        Api.eventActivities.addDocument(newForm).then(this.back);
                    } else if (this.flag === DIALOG.edit.flag) {
                        // 修改人信息
                        newForm.lastModifier = { userId: user.username, userName: user.name };
                        // 编辑活动信息
                        Api.eventActivities.modifyDocument(newForm.id, newForm).then(this.back);
                    }
                }
            });
        },
