<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'brand',name:'文化品牌管理'},{name:'品牌资源'}]"></v-pageheader>
...
export default {
    ...
    methods: {
        loadData() {
            ...
            Api.system.getBrandInfoList(this.objid).then((res) => {
                this.dataList = res || [];
                this.loading = false;
            })
        },
        ...
        del(index, row) {
            let self = this;
            self.delConfirm('品牌资源', () => {
                Api.system.delBrandInfo(this.objid, row.id).then(self.callback);
            });
        },
        // 格式化资源类型
        formatType(row) {
