...
const DIALOG = {
    add: { title: '新增品牌资源', flag: 'add' },
    edit: { title: '编辑品牌资源', flag: 'edit' }
};
...
export default {
    ...
    methods: {
        back() {
            this.$router.push({ path: 'digitinfos_list', query: { id: this.id } });
        },
        //上传封面图
        ...
        submitForm() {
            this.$refs['digitInfoForm'].validate((valid) => {
                ...
                if (this.flag === DIALOG.add.flag) {
                    // this.digitInfoForm.creator = this.$store.getters.AuditUser;
                    // this.digitInfoForm.createTime = this.formatDate(new Date(), 'yyyy-MM-dd HH:mm:ss');
                    Api.system.createBrandInfo(this.id, this.digitInfoForm).then(this.callback);
                } else {
                    // this.digitInfoForm.lastModifier = this.$store.getters.AuditUser;
                    // this.digitInfoForm.lastModifiedTime = this.formatDate(new Date(), 'yyyy-MM-dd HH:mm:ss');
                    Api.system.modifyBrandInfo(this.id, this.did, this.digitInfoForm).then(this.callback);
                }
            });
        },
        getDetail() {
            this.btnload = true;
            Api.system.getBrandInfo(this.id, this.did).then((info) => {
    ...
    created() {
        ...
        this.breadcrumbs = [
            { to: 'brand', name: '文化品牌管理' },
            { to: { path: 'digitinfos_list', query: { id: this.id } }, name: '品牌资源' },
            { name: this.title }
        ]
    }
}
