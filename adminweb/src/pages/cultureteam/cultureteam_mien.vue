<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'index',name: '文化团队管理' },{name:'管理团队风采'}]"></v-pageheader>
...
export default {
    ...
    methods: {
       ...
       loadData() {
            ...
            let str = '';       
            Api.cultureteam.getCultureTeamMien(this.objid).then((res) => {
                if (title !== '')
                {
        ...
        del(index, row) {
            ...
            self.delConfirm('团队风采', () => {
                Api.cultureteam.dellMien(this.objid, row.id).then(self.callback);
            });
        }
    },
    mounted() {
