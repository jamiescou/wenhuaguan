    ...
    <div class="edit-wrapper">
        ...
        <div class="form-wrapper">
            <el-form ref="societyForm" :model="societyForm" :rules="rules" label-position="right" label-width="100px" class="m-form">
                <el-row :gutter="20">
                    <el-col :span="14">
                        <el-form-item label="学会名称" prop="name">
...
const DIALOG = {
    add: { title: '新增群文学会', flag: 'add' },
    edit: { title: '编辑群文学会', flag: 'edit' }
};
export default {
    data() {
        return {
    ...
    methods: {
            ...
            this.$refs['societyForm'].validate((valid) => {
                if (valid) {
                    ...
                    if (this.flag === DIALOG.add.flag) {
                        ...
                        Api.society.addSociety(newForm).then(this.callback);
                    } else if (this.flag === DIALOG.edit.flag) {
                        Api.society.editSociety(this.id, newForm).then(this.callback);
                    }
                }
            })
        },
        ...
        getDetail() {
            Api.society.getSociety(this.id).then((res) => {
    ...
    mounted() {
      ...
      this.$store.getters.user.orgUnit.id
    }
}
</script>
