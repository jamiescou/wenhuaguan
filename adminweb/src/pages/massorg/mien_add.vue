<template>
    <div class="mien-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'index',name:'群团组织管理'},{name:'风采管理'},{name:title}]"></v-pageheader>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      this.digitInfoForm.createTime = time;
      this.$refs["digitInfoForm"].validate(valid => {
        if (valid) {
          let newForm = Object.assign({}, this.digitInfoForm);
          if (this.flag === "add") {
            Api.massorg.addMien(this.id, newForm).then(this.callback);
          } else {
            Api.massorg
              .editMien(this.id, this.$route.query.mid, newForm)
              .then(this.callback);
          }
        }
      });
    ...
    getDetail() {
      Api.massorg.detailMien(this.id, this.$route.query.mid).then(res => {
