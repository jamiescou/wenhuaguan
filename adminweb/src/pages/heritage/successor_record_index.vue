<template>
    <div class="list-wrapper activity-wrapper">
        <v-pageheader :breadcrumbs="[{name: '传承人管理' },{name: '数字资源' }]"></v-pageheader>
            ...
            <el-table :data="dataListOne" border stripe v-loading.body="loading" tooltip-effect="custom-effect">
                ...
                <el-table-column label="资源名称" prop="name" width="260px">
                    <template scope="scope">
                        <router-link :to="{path:'successorViewrecord', query: {id:id,did: scope.row.id}}" class="u-link">
                            {{scope.row.name}}
                        </router-link>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        loadData() {
            ...
            this.showLoading();
            Api.heritage.successorDigicInfos(this.id).then((res) => {
        ...
        // 编辑
        handleEdit(row) {
            this.$router.push({ path: 'successorRecordadd', query: { id: this.id, did: row.id, flag: 'edit', title: '编辑' } });
        },
        // 删除数字资源
        handleDel(row) {
            let self = this;
            self.delConfirm('资源', () => {
                Api.heritage.successorDigicInfoDelete(this.id, row.id).then((res) => {
                    this.showTip();
                    this.loadData();
                });
            });
        ...
        // 添加
        handleAdd() {
            this.$router.push({ path: 'successorRecordadd', query: { id: this.id, flag: 'add', title: '添加' } });
        },
        // 格式化资源类型
        formatType(row) {
            let type = row.type;
