<template>
    <div class="edit-wrapper">
        <v-pageheader :breadcrumbs="[{ to:'index',name:'传承人管理'},{name:'数字资源'},{name:title}]"></v-pageheader>
        ...
        */
        ...
        submitForm() {
            this.$refs['digitInfoForm'].validate((valid) => {
                if (valid) {
                    if (this.flag === 'add') {
                        Api.heritage.successorAddDigicInfo(this.id, this.digitInfoForm).then(this.callback);
                    } else {
                        Api.heritage.successorDigicInfoModify(this.id, this.did, this.digitInfoForm).then(this.callback);
                    }
                }
            });
        },
