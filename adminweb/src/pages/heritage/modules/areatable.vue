<template>
    <div class="table-container">
        <el-table :data="dataList" border stripe v-loading.body="loading" tooltip-effect="custom-effect">
            ...
            <el-table-column label="保护区名称" align="center">
                <template scope="scope">
                    <router-link :to="{path:'areaview',query:{id: scope.row.id,flag:'area'}}" class="u-link">
                        {{scope.row.name}}
                    </router-link>
                </template>
            </el-table-column>
            <el-table-column prop="protectSubject" label="保护主体" align="center"></el-table-column>
            ...
            <el-table-column label="操作" width="300px" align="center">
                <template scope="scope">
                    ...
                    <div v-if="myFlag === 'publish'">
                        ...
                        <a class="btn-act" @click="handleRecommend(scope.row)" v-if="hasOffline(scope.row.onlineStatus)">{{scope.row.isTop?'取消置顶':'置顶'}}</a>
                        <a class="btn-act" @click="handleRecycle(scope.row)" v-if="hasRecycled(scope.row.onlineStatus)">回收</a>
                    </div>
...
export default {
    ...
    methods: {
        ...
        loadData() {
            ...
            Api.heritage.getHeritageAreaList(this.search + str, this.page, this.size).then((res) => {
        ...
        // 编辑
        handleEdit(row) {
            this.$router.push({ path: 'areaadd', query: { id: row.id } });
        },
        // 改变状态
        changeStatus(id, data) {
            ...
            Api.heritage.statusHeritageArea(id, result).then(this.callback).catch();
        },
        // 提交审核
        handleVerify(row) {
        ...
        handleDel(row) {
            let self = this;
            self.delConfirm('保护区信息', () => {
                Api.heritage.delHeritageArea(row.id).then(self.callback);
            });
        },
        // 推荐/ 取消推荐
        handleRecommend(row) {
            Api.heritage.topHeritageArea(row.id, !row.isTop).then(this.callback).catch();
        },
        ...
        // 数字资源
        handleRecord(row) {
            this.$router.push({ path: '../heritageRecord', query: { id: row.id } });
        },
        getFlows(flows) {
            let operDesc = null;
