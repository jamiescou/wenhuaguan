<template>
    <div class="table-container">
        <el-table :data="dataList" border stripe v-loading.body="loading" tooltip-effect="custom-effect">
            ...
            <el-table-column label="非遗资源名称" align="center">
                <template scope="scope">
                    <router-link :to="{name:'viewheritage',params: { id: scope.row.id },query:{flag:myFlag}}" class="u-link">
                        {{scope.row.name}}
                    </router-link>
                </template>
            </el-table-column>
            <el-table-column prop="region" label="区域" align="center" :formatter="convertRegion"></el-table-column>
            <el-table-column prop="type" label="非遗类型" align="center" :formatter="convertType"></el-table-column>
            ...
            <el-table-column label="操作" width="300px" align="center">
                <template scope="scope">
                    ...
                    <div v-if="myFlag === 'publish'">
                        ...
                        <a class="btn-act" @click="handleRecommend(scope.row)">{{scope.row.isTop==1?'取消置顶':'置顶'}}</a>
...
export default {
    ...
    methods: {
        ...
        loadData() {
            ...
            Api.heritage.getHeritageList(this.search + str, this.page, this.size).then((res) => {
        ...
        // 编辑
        handleEdit(row) {
            this.$router.push({ path: 'heritage', query: { id: row.id, flag: this.myFlag } });
        },
        // 改变状态
        changeStatus(id, data) {
            ...
            Api.heritage.modifyHeritageStatus(id, result).then(this.callback).catch();
        },
        // 提交审核
        handleVerify(row) {
        ...
        handleDel(row) {
            ...
            self.delConfirm('名录信息', () => {
                Api.heritage.delHeritage(row.id).then(self.callback);
            });
        },
        // 推荐/ 取消推荐
        handleRecommend(row) {
            Api.heritage.setTop(row.id, !row.isTop).then(this.callback).catch();
        },
        getRegion() {
