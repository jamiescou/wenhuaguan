<template>
    <div class="list-wrapper">
        <v-pageheader :breadcrumbs="[{ name: '文艺人才管理' }]"></v-pageheader>
        <div class="right-opers">
            <el-button type="primary" @click="handleAdd">添加文艺人才</el-button>
        </div>
            ...
            <el-table :data="dataList" border stripe v-loading.body="loading">
                <el-table-column prop="name" label="文艺人才姓名">
                    <template scope="scope">
                        <router-link :to="{path:'literary_detail', query: {id: scope.row.id,flag:1,type:1}}" class="u-link">
                            {{scope.row.name}}
                        </router-link>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        handleAdd() {
            this.$router.push({ path: 'literary_add', query: { flag: 'add',type:'1'} })
        },
        edit(index, row) {
            this.$router.push({ path: 'literary_add', query: { id: row.id, flag: 'edit',type:'1' } })
        },
        del(index, row) {
            let self = this;
