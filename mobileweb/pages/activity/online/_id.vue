    ...
    <section class="container activity-pre">
        ...
        <mt-tab-container v-model="active">
            <mt-tab-container-item id="preInfo">
                <v-seat ref="seatWarpper" :perAllow="detail.perAllow - reservedCount" :preSeats="currentItm.reserveSeats" :seatTmp="detail.seatTemplate" @seatChange="seatChange"></v-seat>
                <section class="pre-info">
                    <div class="flex-item border-bottom reserve-type">
                        <div class="cell fixed title">已选座位&nbsp;</div>
                        <div class="cell">
                            <span v-for="(seat, i) in seatList" :key="'pre_'+i" class="tag">{{seat.seatNo}}</span>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        seatChange(checkedSeats) {
            this.seatList = checkedSeats;
            this.tickCount = this.seatList.length;
        },
        handleNextStep() {
            ...
            if (this.seatList.length > this.currentItm.remainTicket) {
        ...
        async handleConfirm() {
            let seats = this.seatList.map((x) => { return { seatNo: x.seatNo } });
            ...
            let orderInfo = {
                ...
                reserveSum: this.seatList.length,
                seats: seats
            }
            ...
            if (data.success) {
                ...
                this.showMsg(data.message || '预订失败');
            }
        }
    }
}
