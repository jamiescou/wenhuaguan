  ...
  <section class="container project-detail">
    <div class="card base-info">
      ...
      <div class="card-bd">
        ...
        <p class="card-info">
          {{ detailInfo.levelName }}&nbsp;&nbsp;&sdot;&nbsp;&nbsp;{{detailInfo.batchName}}
        </p>
      </div>
    </div>
    <div class="split"></div>
    <mt-navbar v-model="currentTab" class="border-bottom project-nav">
      <mt-tab-item id="1" class="v-line">非遗名录介绍</mt-tab-item>
    ...
    <div class="comments">
      ...
      <div class="more border-top">
        <nuxt-link :to="{path: '/comments/'+detailInfo.id,query:{type:'project'}}">
...
export default {
  ...
  async asyncData({ params, error, req, query }) {
    let detailInfo = await axios.get('/project/detail/' + query.id);
    let comments = await axios.get('/comments/project/' + query.id + '/0?size=1');
    return {
