 ...
 const path = require('path')
     //  const fetchFiles = require('node-fetch');
 ...
 router.get('/', async(ctx, next) => {
     ...
     let res = await eventActivities.collectList(search, page, config.paging.collect.size);
     for (let i = 0; i < res.content.length; i++) {
         let item = res.content[i];
         item.coverPic = assist.getFileUrl(item.coverPic);

         let works = await eventActivities.workList('isPass:true,activity.id:' + item.id, 0, -1);
     ...
     await ctx.render('eventActivities/index', {
         data: res,
         query: ctx.query
     })
 })
 ...
 router.get('/collectdetail/:id', async(ctx, next) => {
     ...
     let [works, comments, collects] = await Promise.all([
         eventActivities.workList(search, 0, -1),
         users.getComments('PhotographyArtWorks', ctx.params.id, commentPage, config.paging.comments.size),
         eventActivities.collectList('search=onlineStatus:Published', 0, -1) //将所有的征集活动取出来
     ])
     ...
     let thumed = false;
     if (ctx.session.user) {
         let favStatus = await users.fetchUserFavoritesStatus(ctx.session.user.id, 'PhotographyArtWorks', ctx.params.id);
         let thumStatus = await users.fetchUserThumbup(ctx.session.user.id, 'PhotographyArtWorks', ctx.params.id);
     ...
     await eventActivities.addvisit(ctx.params.id);
     await ctx.render('eventActivities/collectdetail', {
         data: res,
         favorited: favorited,
         works: works,
         nextPage: nextPage,
 ...
 router.post('/uploadfile', async(ctx, next) => {
     ...
     let response;
     try {
         // response = await eventActivities.fileUploadZip(formData);
         const url = config.apiServer.host + '/api/v1/photographyCollect/works/uploadAndThumbnailsWorks'
         ...
         let options = {
             ...
             uri: url,
             formData: {
                 fileName: fileInfo.fields.name,
                 file: {
                     value: fs.createReadStream(fileInfo.files.file.path),
                     options: {
                         filename: fileInfo.fields.name,
         ...
         let filePath = assist.getFileUrl(responseData.thumbnailsUrl);
         ctx.body = {
             "code": "0",
             "res": responseData.thumbnailsUrl, //文件相对地址(缩略图),
             'originalFilePath': responseData.originalDrawingUrl, // 原图
 ...
 router.get('/booking/:id', async(ctx, next) => {
     let [res] = await Promise.all([
         eventActivities.collectDetail(ctx.params.id),
     ])
     res.coverPic = assist.getFileUrl(res.coverPic);
     res.endTime = res.endTime.substr(0, 16);
     res.startTime = res.startTime.substr(0, 16);
     await ctx.render('eventActivities/booking', {
         data: res,
 ...
 router.post('/order/:id', async(ctx, next) => {
         ...
         let workObj = {
             ...
             "files": [{
                 "fileType": "pic",
                 "filePath": url,
                 "originalFilePath": workInfo.fields.bigInputfile
             }],
             "isPass": ""
         }
         try {
             let resupload = await eventActivities.workUpload(workObj);
             logUtil.logbusiness([ctx.session.user.mobile, 'uploadWorks', activityId]);
             ctx.redirect('/eventActivities/success/' + ctx.params.id);
         } catch (ex) {
             ctx.redirect('/eventActivities/booking/' + ctx.params.id);
 ...
 router.get('/success/:id', async(ctx) => {
     let res = await eventActivities.collectDetail(ctx.params.id);
     res.coverPic = assist.getFileUrl(res.coverPic);

     await ctx.render('eventActivities/success', {
         data: res,
         activityId: ctx.params.id
     })
