...
class TaskModel {
    ...
    async fetchVolunrecruitList(search, page, size) {
        let res = await fetch({
            url: '/volunteer/volunrecruit?' + search + '&page=' + page + '&size=' + size+'&sort=createTime~desc',
            method: 'get'
        });
        return res;
    }
     ...
     //获取志愿者团队列表
     async fetchVolunList(search, page, size) {
        let res = await fetch({
            url: '/volunteer/volunrecruit?' + search + '&page=' + page + '&size=' + size+'&sort=createTime~desc',
            method: 'get'
        });
    ...
    async fetchVolunteerDetail(id) {
      ...
      async teammien(id) {
        let res = await fetch({
            url: '/volunteer/volunteams/' + id+'/spirits',
            method: 'get'
        });
        return res;
    }
    ...
    //查询志愿者
    async volunrecruitInfo(search,page,size) {
        const data = await fetch({
    ...
    async isbook(id,userid) {
        const data = await fetch({
            url: '/volunteer/volunapplys?search=objId:'+id,
            method: 'get',
        });
        return data;  
    }
    async postActivitySave(id, info) {
     ...
     async joinactivity(userInfo) {
      ...
      async getRecruitjoinapplys(search,page,size){
        const data = await fetch({
            url: '/volunteer/recruitjoinapplys?search='+search+'&page='+page+'&size='+size+'&sort=applyTime~desc',
            method: 'get',
    ...
    async postComments(user, id) {
        let res = await fetch({
            url: '/volunteer/volunrecruit/' + id + '/comments',
            method: 'post',
            data: user
        });
        return res;
    ...
    async getComments(id, page, size) {
        let res = await fetch({
            url: '/volunteer/volunrecruit/' + id + '/comments?page=' + page + '&size=' + size + '&sort=time~desc',
            method: 'get'
